...
├── openclaw_demo.py        # OpenClaw 面试展示 CLI：输出 Markdown / JSON 报告
...
├── openclaw/
│   └── talentmatch-ai/
│       └── SKILL.md        # OpenClaw skill 使用说明与安全边界
└── tests/                  # 核心逻辑测试
```
...
本项目新增了 OpenClaw 可调用的 Agent 技能包与 CLI 工作流，适合给 AI-HR 面试官演示“Agent 自动化闭环”：

```text
JD 解析 → 搜索策略生成 → 100 人候选池召回 → 匹配评分 → Top 候选人解释 → 触达草稿 → 合规说明
```
...
- 只使用本地 mock 数据、CSV 或用户主动提供的 JD 文本。
- 不登录招聘平台。
- 不抓取页面，不绕过验证码、权限或风控。
- 不自动发送触达消息。
- 不读取、打印、导出 `.env` 或 API Key。
- 所有触达草稿都标记为 `待人工确认`。
...
```text
openclaw/talentmatch-ai/SKILL.md
```

可以将该目录作为 OpenClaw workspace skill 使用。它会指导 OpenClaw Agent 在合规边界内调用本项目 CLI，并将输出总结成面试官可理解的招聘 Copilot 演示。
...
JSON 输出，适合让 OpenClaw Agent 继续解析和总结：
...
```text
用 TalentMatch AI 帮我演示一个 AI 产品经理岗位的候选人筛选闭环，输出 Top 10 候选人、关键证据、触达草稿和合规护栏。
```

或：

```text
Run the TalentMatch AI OpenClaw demo in JSON mode, then summarize the top 3 candidates for an AI-HR interviewer.
```

### 面试展示话术

> 这个 OpenClaw 版本把 TalentMatch AI 从 Web Demo 变成了 Agent 可调用的招聘工作流。Agent 可以从 JD 出发，自动生成搜索策略，在 100 人 mock 候选池中完成召回和排序，再输出证据级解释与待人工确认的触达草稿。它展示的是 AI-HR 场景里“自动化判断链路”，但不做真实平台抓取和自动发送。
...
- 在 OpenClaw 中演进为可组合 skill：`job_intake`、`source_connector`、`candidate_ranker`、`outreach_draft`、`crm_followup`。
- 支持定时刷新候选人池，但继续保持“真实触达前人工确认”。
...
- **OpenClaw Agent 记忆**：只记录用户授权的偏好、岗位模板和反馈标签，不保存 API Key 或未授权候选人隐私。
...
python3 -B -m unittest tests/test_openclaw_demo.py
```
